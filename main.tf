# groups with members and owners
resource "azuread_group" "main" {
  for_each = {
    for key, group in var.groups : key => group
    if !coalesce(group.ignore_members, false) && !coalesce(group.ignore_owners, false)
  }
...
# groups without members and owners
resource "azuread_group_without_members" "manual" {
  for_each = {
    for key, group in var.groups : key => group
    if coalesce(group.ignore_members, false) || coalesce(group.ignore_owners, false)
  }
